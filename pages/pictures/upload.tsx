...
const Upload: NextPage = () => {
  ...
  const [uploadProduct, { loading, data }] =
    useMutation<UploadProductMutation>("/api/pictures");
  const onValid = async ({
    name,
    age,
    mbti,
  ...
  useEffect(() => {
    if (data?.ok) {
      router.replace(`/pictures/${data.product.id}`);
    }
  }, [data, router]);
  ...
  return (
    <Layout canGoBack title="Upload Picture" seoTitle="Post Upload">
      <form className="space-y-4 p-4" onSubmit={handleSubmit(onValid)}>
        <div>
          {photoPreview ? (
            <img
              src={photoPreview}
              className="h-48 w-full rounded-md text-gray-600 "
            />
          ) : (
        ...
        <TextArea
          ...
          required
        />
        <Button text={loading ? "Loading..." : "Upload Picture"} />
      </form>
    </Layout>
  );
};
